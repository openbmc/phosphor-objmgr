				       ...
				       _unused_ uint64_t usec, void *userdata)
{
	...
	r = sd_bus_call_method_async(wait->conn, NULL,
				     "xyz.openbmc_project.ObjectMapper",
				     "/xyz/openbmc_project/object_mapper",
				     "xyz.openbmc_project.ObjectMapper",
				     "GetObject", async_wait_getobject_callback,
				     data, "sas", data->path, 0, NULL);
	if (r < 0) {
					 ...
					 _unused_ sd_bus_error *e)
{
	...
	if ((r == EBUSY || r == ENOBUFS) && data->retry < mapper_busy_retries) {
		...
		next_retry +=
			mapper_busy_delay_interval_usec * (1 << data->retry);
		r = sd_event_add_time(wait->loop, &data->event_source,
					  ...
					  void *userdata)
{
	...
	r = sd_bus_call_method_async(
		...
		async_subtree_getpaths_callback, subtree, "sias",
					   ...
					   _unused_ sd_bus_error *e)
{
	...
	if ((r == EBUSY || r == ENOBUFS) &&
	    subtree->retry < mapper_busy_retries) {
		...
		next_retry +=
			mapper_busy_delay_interval_usec * (1 << subtree->retry);
				  ...
				  mapper_async_subtree **t, int op)
{
	...
	if (subtree->op == MAPPER_OP_REMOVE) {
		r = sd_bus_add_match(conn, &subtree->slot,
			       ...
			       sd_bus_message **reply)
{
	...
	r = sd_bus_message_new_method_call(conn, &request,
					   "xyz.openbmc_project.ObjectMapper",
					   "/xyz/openbmc_project/object_mapper",
					   "xyz.openbmc_project.ObjectMapper",
					   "GetObject");
