                       ...
                       const std::string& typeFilter, bool pathMode)
{
    ...
    {
        if (!nameFilter.empty() &&
            !std::string(key.a).contains(nameFilter) &&
    ...
    if (pathMode)
    {
        ...
        {
            for (const auto& p : info.aToBPaths)
            {
                std::println("{} -> {}", std::string(p), std::string(key.b));
            }
            for (const auto& p : info.bToAPaths)
            {
                std::println("{} -> {}", std::string(p), std::string(key.a));
            }
        }
        return;
    }
