...
struct InProgressIntrospect
{
    InProgressIntrospect(
        ...
        io(io), process_name(processName), assocMaps(am)
#ifdef DEBUG
        ,
    ...
    ~InProgressIntrospect()
    {
        sendIntrospectionCompleteSignal(system_bus, process_name);
        ...
        [&interfaceMap, &objectServer, transaction, path, systemBus,
         timeoutRetries](const boost::system::error_code ec,
            ...
            while (pElement != nullptr)
            {
                ...
                thisPathMap[transaction->process_name].emplace(ifaceName);
...
void startNewIntrospect(
    sdbusplus::asio::connection* systemBus, boost::asio::io_context& io,
    interface_map_type& interfaceMap, const std::string& processName,
    AssociationMaps& assocMaps,
#ifdef DEBUG
    std::shared_ptr<std::chrono::time_point<std::chrono::steady_clock>>
        global_start_time,
...
void doListNames(
    boost::asio::io_context& io, interface_map_type& interfaceMap,
...
std::vector<interface_map_type::value_type>
    getAncestors(const interface_map_type& interfaceMap, std::string reqPath,
...
boost::container::flat_map<std::string, boost::container::flat_set<std::string>>
    getObject(const interface_map_type& interfaceMap, const std::string& path,
              std::vector<std::string>& interfaces)
{
...
std::vector<interface_map_type::value_type>
    getSubTree(const interface_map_type& interfaceMap, std::string reqPath,
...
std::vector<std::string> getSubTreePaths(const interface_map_type& interfaceMap,
