...
struct InProgressIntrospect
{
    ...
    ~InProgressIntrospect()
    {
        try
        {
            ...
            std::chrono::duration<float> diff =
                std::chrono::steady_clock::now() - processStartTime.value();
    ...
    // only relevant for MAPPER_ENABLE_DEBUG
    std::shared_ptr<std::chrono::time_point<std::chrono::steady_clock>>
        globalStartTime;
...
int main()
{
        ...
        if (!newOwner.empty())
        {
            ...
            if (needToIntrospect(name))
            {
                ...
                startNewIntrospect(systemBus.get(), io, interfaceMap, name,
                                   associationMaps, transaction, server);
            }
        }
    };
    ...
    iface->register_method(
        ...
        [&interfaceMap](sdbusplus::object_path& subtreePath1,
                        std::vector<std::string>& interfaces1,
                        std::vector<std::string>& associations,
                        sdbusplus::object_path& subtreePath2,
                        std::vector<std::string>& interfaces2, int32_t depth) {
            return getPathsByAssociation(
                interfaceMap, associationMaps, subtreePath1, interfaces1,
                associations, subtreePath2, interfaces2, depth);
        });
