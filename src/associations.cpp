    ...
    AssociationMaps& assocMaps, sdbusplus::asio::object_server& server)
{
    ...
    for (const auto& [owner, association] : associationData)
    {
        ...
        // Unsure why this is needed. If we are spamming add/remove associations, the pending
        // list will keeps growing and not cleanup until the spam stops... potentially OOM.
