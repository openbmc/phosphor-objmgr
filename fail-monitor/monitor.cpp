...
bool Monitor::isValidAction(const std::string& action)
{
...
void Monitor::runTargetAction()
{
    ...
    if (reply.is_method_error())
    {
        log<level::ERR>("Failed to start/stop the target",
                        entry("TARGET=%s", target.c_str()),
                        entry("METHOD=%s", methodCall));
        throw std::runtime_error("Failed to start/stop the target");
    }
}
